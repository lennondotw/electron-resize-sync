...
Date: 2026-09-24\
Status: resize deadline meets the goal in an experiment (binary-patched Electron); productisation open
...
| Approach                 | How                                                                                                                                                                  | Meets the goal?                                                                                                                                   | Status                                                                                                                                         |
| ------------------------ | -------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------- |
| Renderer only            | React to `resize` in the page                                                                                                                                        | No: the window has already changed when the page hears about it. Matching `html` to `#root` only hides the leak.                                  | Rejected                                                                                                                                       |
| Resize pacing            | Main process cancels `will-resize`, keeps the latest bounds, applies them with `setBounds` after the renderer acks the previous size                                 | No: commit rate follows the renderer, but each size still lands before it is rendered                                                             | Implemented (`resize sync` switch, later `ELECTRON_RESIZE_SYNC_PACING=1`), [measured](../../../experiments/2026-09-24/resize-pacing/README.md) |
| Resize pacing with yield | As resize pacing, and the renderer skips its busy work while a committed size is unrendered                                                                          | Shortens the unpainted time; does not remove it. Changes the workload during resize.                                                              | Not started                                                                                                                                    |
| Render before reveal     | Put the page in a `WebContentsView` sized independently of the window. Growing: enlarge the view (overflow is clipped), wait for its frame, then enlarge the window. | Growing: yes in principle. Shrinking: either the window shrinks first (stale layout clipped) or the view does (background exposed) for one frame. | Not started                                                                                                                                    |
| Resize deadline          | Hold the window's Core Animation transaction until the renderer's frame at the new size is ready, as Chromium on macOS reportedly can during live resize             | Yes in principle, both directions                                                                                                                 | Unverified: whether Electron 44 enables it, its timeout, and whether it can be driven                                                          |

## Decision

Open, with resize deadline as the focus (decided by the user on 2026-09-24):
every approach is to be tried, resize deadline in most depth. Resize pacing
stays behind a switch as a baseline.

Earlier commits, run labels and data files call these approaches by letter:
A renderer only, B resize pacing, B′ (`bprime`) resize pacing with yield,
C render before reveal, D resize deadline.

## Tried so far

- Measurement bridge and HUD readout: `edf186e`.
- Resize pacing: `b378089`; ack carries the rendered size: `8a3483f`.
...
- [Source reading](../../research/2026-09-24/chromium-resize-sync.md) of
  ...
  implementations of resize deadline.
...
- Cost of resize deadline (`a7d1567`, `127541b`): each size step waits for a renderer
...
- [Resize matrix on the built-in display](../../../experiments/2026-09-24/resize-matrix-builtin/README.md)
  (`6ceea54`): resize deadline with the switches set by the app is in step with and
  without busy work, and costs nothing measurable with nothing to render.
  Render before reveal (`9f7010f`) fails when shrinking and is the
  slowest. Resize pacing with yield reduces the error without removing it.

## Next steps

1. Try a continuous drag by hand with the patched build and everything off,
   to judge smoothness directly.
2. Decide how to ship resize deadline: build Electron with the
